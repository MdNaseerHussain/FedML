...
def transmissionCH(packet_loss_prob, k):
...
def fedML(packet_loss_prob, k):
    ...
    for comm_round in range(comms_round):
        ...
        for _, client in enumerate(client_names):
            ...
            transmissions, decodable = transmissionCH(packet_loss_prob, k)
        ...
        with open('output/mds_prob/comm_matrix{}.txt'.format(int(100*packet_loss_prob)), 'a') as f:
...
if __name__ == '__main__':
    p = float(sys.argv[1])
    acc = fedML(p, 20)
    np.savetxt('output/mds_prob/acc{}.csv'.format(int(p*100)), acc, delimiter=",")
